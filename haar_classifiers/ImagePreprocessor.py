...
class ImagePreprocessor:
    ...
    def __init__(self, source_train_dir=None, source_test_dir=None, dest_train_dir=None, dest_test_dir=None):
        ...
        if dest_train_dir is None:
            self.dest_train_dir = 'INRIAPerson/train_haar/'
        else:
            self.dest_train_dir = dest_train_dir

        if dest_test_dir is None:
            self.dest_test_dir = 'INRIAPerson/test_haar/'
        else:
    ...
    def crop_and_save_neg(self, img_paths, is_train):
        for path in img_paths:
            ...
            for i in range(self.neg_crops_per_neg_image):
                ...
                if is_train:
                    # dest = path.replace('train_64x128_H96', 'train_haar')
                    dest = path.replace('train', 'train_haar')
                else:
                    # dest = path.replace('test_64x128_H96', 'test_haar')
                    dest = path.replace('test', 'test_haar')
    ...
    def preprocess_images(self):
        print('preprocess')
        print(os.path.exists(self.source_train_dir + 'pos.lst'))
        ...
        print('train_pos_list: ', len(train_pos_list))
